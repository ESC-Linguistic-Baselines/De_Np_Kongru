...
def build_np_dict():
    np_dict = {"author": {"learner_id": "",
                          "lvl" : ""},
               
               "NP_info": {"file_ID": "",       # nr to id og file
...
def get_TOK_info(np_dict, np, i):
    ...
    tok_id = "TOK" + str(i+1)
    np_dict["TOK_info"][tok_id] = build_TOK_dict()
    ...
    for token in np[6]:
        if token[0] == np[6][i][6] and np[6][i][6] != "0":
            np_dict["syntax_info"][rel_id] = get_syntax_info(np[6][i], np)
...
def get_syntax_info(tok, np):
    ...
    ######### to be refined with morpho info(?) ##################
            ...
            if regens[4] in ["NE", "NN", "ADJ", "DET", "PPOS", "KON"]:
                ...
                relx_dict["target_case"]["requires_case"] = regens[5] #too simplistic in this case
                relx_dict["target_case"]["found_case"] = tok[5]
            ...
            if relx_dict["target_case"]["requires_case"] == relx_dict["target_case"]["found_case"]:
...
def run_script(filename):
    ...
    # read output file
    with open (filename, mode="r", encoding="utf-8-sig") as f:  #utf-8-sig to avoid BOM error
        text = ast.literal_eval(f.read().strip())               #ast.literal_eval: interpret string as python syntax
        ...
        if len(sent) > 1:
            ...
            for np in sent:
                ...
                for i, tok in enumerate(np[6]):
                    np_dict_TOK = get_TOK_info(single_np_dict, np, i)
...
#### example output for nps in sentence ##########
    for x in all_nps_infile[5]: print(x, "\n")
...
if __name__ == "__main__":
    ...
    myfile = "/Users/christopherchandler/repo/Python/small_projects/De_NP_Kongru/dp_np_kongru/app_resources/corpus/data_nps_extracted/1023_0001416_output_exported_nps.txt"
    run_script(myfile)
